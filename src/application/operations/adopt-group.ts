...
export interface AdoptedGroupValue extends ResolvedAdoption {
  type: "group";
  action: "created" | "updated" | null;
}

export type AdoptGroupsResult = OperationResult<{
  groups: AdoptedGroupValue[];
...
export async function runAdoptGroups(
  ...
  return (dependencies.lock ?? mutationLock).runExclusive(project.statePath, async () => {
    ...
    if (ids.length > 0) {
      ...
      const groupTypeId = await resolveGroupTypeId(opts.groupType, client);
    ...
    if (resolvedIds.length === 0) {
      return {
        ...
        project,
        warnings,
        value: { groups: [], configBlock: null, noMatches: true, dryRun: opts.dryRun ?? false },
      };
    }
      ...
      if (opts.withDynamic) {
        ...
        if (captured) {
          ...
          if (opts.portableRulesets !== false) {
            ...
            const { ruleset, warnings: portableWarnings } = portablizeRuleset(captured.normalizedRuleset, {
            ...
            if (portableWarnings.length > 0) {
              const lines = formatPortablizeWarnings(portableWarnings);
              if (opts.strictRulesets) {
                throw new Error(
                  `--strict-rulesets: ${relPath} would contain ${portableWarnings.length} unportablized ` +
          ...
          if (!opts.dryRun) {
            await mkdir(join(project.cwd, "rulesets"), { recursive: true });
            await writeFile(
              join(project.cwd, relPath),
    ...
    if (opts.dryRun) {
      return {
        ...
        warnings,
        value: {
          groups: results.map((result) => ({ ...result, type: "group", action: null })),
          configBlock: null,
          noMatches: false,
          dryRun: true,
        },
    ...
    return {
      ...
      value: {
        groups: results.map((result) => ({
          ...result,
          type: "group",
          action: reports.find((report) => report.id === result.id)?.action ?? null,
        })),
        configBlock: block,
        noMatches: false,
        dryRun: false,
