...
describe("desiredTuples", () => {
  ...
  it("accepts an admin-authored authId >= 10000 member right on group_type_role (#65)", () => {
    ...
    expect(
      desiredTuples(
        { key: "t", domainType: "group_type_role", domainId: 8, grants: ["churchdb:+add person"] },
        state,
      ),
    ...
    const tuples = desiredTuples(
      {
        ...
        domainType: "group_type_role",
        domainId: 8,
        grants: [{ right: "churchgroup:view group", scope: ["kids_area", "other"] }],
      },
      state,
    );
      ...
      desiredTuples(
        {
          ...
          domainType: "group_type_role",
          domainId: 8,
          grants: [{ right: "churchcore:administer settings", scope: ["kids_area"] }],
        },
        state,
      ),
    ...
    const tuples = desiredTuples(
      {
        ...
        domainType: "group_type_role",
        domainId: 8,
        grants: [{ right: "churchdb:view comments", scope: [1, 2] }],
      },
      state,
    );
    expect(tuples).toEqual([
    ...
    const tuples = desiredTuples(
      {
        ...
        domainType: "group_type_role",
        domainId: 8,
        grants: [{ right: "churchgroup:view group", scope: ["kids_area", 3] }],
      },
      state,
    );
    expect(tuples).toEqual([
...
describe("buildPermissionPlan", () => {
  ...
  it("resolves a group_role domain by (group, role) reference and reconciles idempotently (#25)", async () => {
    const client = {
      get: vi.fn(async (path: string) => {
        ...
        if (path === "/permissions/group_role")
          return [
            {
  ...
  it("warns when the instance CT version differs from the catalog's recorded version (#25)", async () => {
    ...
    expect(
      warnings.some((w) =>
        /catalog was captured from ChurchTools .* but this instance\s+runs 9\.99\.0/is.test(w),
      ),
    ).toBe(true);
  });
